...
export default function FinalPodium({ entries = [], variant = 'default' }) {
  ...
  return (
    <div className={displayMode ? 'space-y-8' : 'space-y-5'}>
      <ResultPanel tone="success" className={displayMode ? 'projector-panel border-amber-300/20 bg-amber-300/10 p-8 lg:p-12 dark:border-amber-300/20 dark:bg-amber-300/10' : ''}>
        ...
        <div className={displayMode ? 'mt-12 grid gap-5 md:grid-cols-3 md:items-end' : 'mt-6 grid gap-3 md:grid-cols-3'}>
          {orderedTopThree.map((entry) => {
            ...
            <div
              ...
              className={`relative overflow-hidden rounded-lg border text-center dark:bg-zinc-950/70 ${
                isWinner
                  ? `border-amber-300 bg-amber-50 dark:border-amber-700 ${displayMode ? 'p-9 md:-mt-14 md:pb-12' : 'p-5 md:-mt-4 md:pb-7'}`
                  : `border-green-200 bg-white/80 dark:border-green-800 ${displayMode ? 'p-7 opacity-95' : 'p-5'}`
              }`}
            >
              ...
              <div className="mt-4 flex justify-center">
                <PigeonAvatar avatar={entry.avatar} size={displayMode ? (isWinner ? 168 : 126) : isWinner ? 'xl' : 'lg'} label={`Avatar de ${entry.name}`} />
              </div>
              <p className={displayMode ? `${isWinner ? 'text-4xl' : 'text-3xl'} mt-5 font-black text-gray-950 dark:text-white` : 'mt-4 text-lg font-bold text-gray-950 dark:text-white'}>
                {entry.name}
              </p>
